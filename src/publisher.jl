...
function publish(publisher, urn, textdir, imgwidth=150)
    # 1. Filter all texts cataloged as online for requested urn:
    ...
    elseif length(catalogrows) > 1
        ...
        # 2. Filter DSE records for this text;
        dse = dse_df(publisher.repo)      
        dserows  = filter(row -> urncontains(urn, row.passage), dse)
        # 3. Read configuration and generate editions:
        ...
        # 4. Paste together the pieces of the markdown page:
        thumb = ""
        if nrow(dserows) > 0
